...
router.get('/', auth, async (req, res) => {
    try {
        const [rows] = (await pool.query('SELECT category_id, `limit` FROM budgets WHERE user_id = ?', [req.user.id]));
        res.json(rows.map((r) => ({
...
router.put('/', auth, async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO budgets (user_id, category_id, `limit`) VALUES (?, ?, ?) ' +
            'ON DUPLICATE KEY UPDATE `limit` = VALUES(`limit`)', [req.user.id, categoryId, new Decimal(limit).toFixed(2)]);
        res.json({ ok: true });
    }
    catch (e) {
...
router.get('/reports/monthly', auth, async (req, res) => {
    try {
        ...
        const [rows] = (await pool.query(`SELECT DATE_FORMAT(CONVERT_TZ(date, '+00:00', '+05:30'), '%Y-%m') AS month,
