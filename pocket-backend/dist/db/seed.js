...
async function seed() {
    ...
    for (const cat of SYSTEM_CATEGORIES) {
        await pool.query('INSERT INTO categories (id, user_id, name, type, icon, color) VALUES (?, NULL, ?, ?, ?, ?) ' +
            'ON DUPLICATE KEY UPDATE name = VALUES(name), icon = VALUES(icon), color = VALUES(color)', [cat.id, cat.name, cat.type, cat.icon, cat.color]);
    }
