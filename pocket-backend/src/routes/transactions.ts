...
type TransactionInput = z.infer<typeof txSchema>;
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    res.json((rows as any[]).map(rowToTx));
  } catch (e) {
...
router.post('/', auth, async (req, res) => {
  ...
  try {
    const data = txSchema.parse(req.body) as TransactionInput;
    ...
    await connection.query(
      `INSERT INTO transactions (id, user_id, title, amount, type, category_id, date, note, receipt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, req.user!.id, data.title, amount, data.type, data.categoryId, date, data.note ?? null, data.receipt ?? null]
    );
...
router.patch('/:id', auth, async (req, res) => {
  const connection = await pool.getConnection();
  try {
    const patch = txSchema.partial().parse(req.body) as Partial<TransactionInput>;
    ...
    for (const [k, col] of Object.entries(map)) {
      ...
      if (val !== undefined) {
        fields.push(`${col} = ?`);
        if (k === 'date') values.push(toUTC(val));
        else if (k === 'amount') values.push(new Decimal(val).toFixed(2));
