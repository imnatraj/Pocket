...
router.post('/', auth, async (req, res) => {
  try {
    ...
    await pool.query(
      'INSERT INTO recurring_transactions (id, user_id, title, amount, type, category_id, frequency, start_date, next_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, req.user!.id, data.title, amount, data.type, data.categoryId, data.frequency, startDate.toISOString(), nextDate]
    );
